defmodule MaxoNew.Generator do
  @moduledoc false
  ...
  def copy_from(%Project{} = project, mod, name) when is_atom(name) do
    ...
    for {format, project_location, files} <- mapping,
      ...
      cond do
        ...
        format in [:text, :eex] ->
          create_file(fs, target, mod.render(name, source, project.binding))

        format == :config ->
          contents = mod.render(name, source, project.binding)
          config_inject(fs, Path.dirname(target), Path.basename(target), contents)

        format == :prod_config ->
          contents = mod.render(name, source, project.binding)
          prod_only_config_inject(fs, Path.dirname(target), Path.basename(target), contents)
      end
    end
  end

  def config_inject(fs, path, file, to_inject) do
    file = Path.join(path, file)

    contents =
      case Virtfs.read(fs, file) do
        {:ok, bin} -> bin
    ...
    with :error <- split_with_self(contents, "use Mix.Config"),
      ...
      [left, middle, right] ->
        write_formatted!(fs, file, [left, middle, ?\n, ?\n, to_inject, right])
    end
  end

  def prod_only_config_inject(fs, path, file, to_inject) do
    file = Path.join(path, file)
  ...
  defp write_formatted!(fs, file, contents) do
    formatted = contents |> IO.iodata_to_binary() |> Code.format_string!()
    Virtfs.write!(fs, file, [formatted, ?\n])
  end
  ...
  def gen_ecto_config(%Project{project_path: project_path, binding: binding, fs: fs}) do
    adapter_config = binding[:adapter_config]

    config_inject(fs, project_path, "config/dev.exs", """
    # Configure your database
    config :#{binding[:app_name]}, #{binding[:app_module]}.Repo#{kw_to_config(adapter_config[:dev])}
    """)

    config_inject(fs, project_path, "config/test.exs", """
    # Configure your database
    #
    ...
    prod_only_config_inject(fs, project_path, "config/runtime.exs", """
